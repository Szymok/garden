...
---
# 🎯 Definicja
Mechanizm aplikowania zmian w modelu metadanych ([[Metadata|Metadata]] Model) lub aktualizacji wersji systemu, wymagający przełączenia aplikacji w tryb serwisowy ("No Model Mode").
...
- **[[Backup|Backup]]:** System wymusza potwierdzenie wykonania snapshotu [[Bazy danych|bazy danych]] przed rozpoczęciem procedury.

# 📚 Szczegółowe wyjaśnienie
Kiedy Data Steward zmienia model (np. dodaje pole "NIP" do encji Klient), zmiana ta musi zostać odzwierciedlona w strukturze [[Bazy danych|bazy danych]]. Nie dzieje się to "w locie".
Proces wygląda tak:
...
Aktualizacja wersji [[Ataccama|Ataccama]] ONE z 14.5 do 14.6:
...
## 👽 Brudnopis
There are two types of system changes: System upgrade commands (new system version) and User changes ([[Metadata|metadata]] model changes).
In both cases, [[Metadata|metadata]] model changes are published and propagated to the database only after the [[Metadata|metadata]] model has been successfully validated.
**Procedure**:
...
5. **[[Backup|Backup]] Confirmation** (Create DB snapshot is strongly recommended/mandatory step).
