...
---
# 🎯 Definicja
Możliwość wykorzystania usług bazodanowych zarządzanych przez dostawcę chmury ([[PaaS|PaaS]]), takich jak AWS RDS lub Azure Database for PostgreSQL, jako repozytorium dla platformy [[Ataccama|Ataccama]] ONE, zamiast instalacji PostgreSQL na maszynie wirtualnej.
...
Instalator Ansible nie tworzy instancji RDS (nie używa [[Terraform|Terraform]]). Oczekuje gotowego adresu `host:port` oraz użytkownika z uprawnieniami `CREATEDB`/`CREATEROLE` (lub stworzonymi wstępnie bazami).
...
## 📌 Źródła
- [[Ataccama|Ataccama]] Cloud Deployment Guide.

## 👽 Brudnopis
- Managed Service vs Self-Hosted: Managed droższe, ale mniej pracy.
- OpenSearch: [[Ataccama|Ataccama]] wymaga specyficznych pluginów, więc AWS OpenSearch Service często nie działa (wymagany self-hosted na EC2).
