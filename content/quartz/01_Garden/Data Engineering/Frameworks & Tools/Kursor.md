...
- **[[Airbyte|Airbyte]] automatycznie używa wartości kursora w zapytaniach** do systemu źródłowego – pobierając tylko świeże rekordy.
...
1. **Pierwsza synchronizacja:**
    - [[Airbyte|Airbyte]] pobiera wszystkie rekordy ze źródła i zapisuje najwyższą wartość kursora.
...
# 💡 Przykład zastosowania w [[Airbyte|Airbyte]]

**Przykład 1:**  
Synchronizacja bazy Postgres z hurtownią [[Snowflake|Snowflake]]:

- Pole kursor: `last_modified`
- [[Airbyte|Airbyte]] rejestruje max(`last_modified`) po każdym tasku.
- Przy kolejnym uruchomieniu pipeline, pobiera tylko rekordy gdzie `last_modified` > ostatni kursor.

**Przykład 2:**  
API do systemów [[SaaS|SaaS]] (np. CRM):

- API często udostępnia pole `lastSyncToken` lub `since_timestamp`.
- [[Airbyte|Airbyte]] automatycznie uzupełnia wartość kursora na podstawie poprzedniego eksportu.
...
- Standard nie tylko w [[Airbyte|Airbyte]], ale też Fivetran, Informatica, Talend, Apache NiFi
...
- W [[Airbyte|Airbyte]] obsługa field cursor na poziomie connectora i konfigurowalna w UI