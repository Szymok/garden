...
List entities
You can retrieve an entity, including its content and the embedded attributes, by the entity identifier. The operation also lets you query deep structures such as embedded entities and references. It is also possible to apply: • primitive ordering (orderBy: [{ property: "name", direction: ASC } l) • pagination (skip, size, cursor) e filtering (filter for AQL, query for AQL that is preprocessed by AI, queryPartss for user-friendly filters which are processed by Al).

List catalog items
• TO view all catalog items that you can access, use the following query. • The query retrieves the identifier and name of each catalog item, alongside the identifiers and names of their respective attributes.
...
List data sources • To list all data sources and their data source connections, run the following query:
...
List monitoring projects • TO run a monitoring project, you need the monitoring project identifier (projectld). • All project identifiers can be retrieved through the following query. GraphQL query for listing monitoring projects query getProjects { monitoringProjects(versionSeIector. { draftVersion: true { edges { node { gid
