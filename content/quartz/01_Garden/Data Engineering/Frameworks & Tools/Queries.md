...
List entities
You can retrieve an entity, including its content and the embedded attributes, by the entity identifier. The operation also lets you query deep structures such as embedded entities and references. It is also possible to apply: • primitive ordering (orderBy: [{ property: "name", direction: ASC } l) • pagination (skip, size, cursor) e filtering (filter for [[AQL|AQL]], query for [[AQL|AQL]] that is preprocessed by AI, queryPartss for user-friendly filters which are processed by Al).

List [[Catalog Items|catalog items]]
• TO view all [[Catalog Items|catalog items]] that you can access, use the following query. • The query retrieves the identifier and name of each catalog item, alongside the identifiers and names of their respective attributes.
...
List data sources • To list all data sources and their [[Data Source|data source]] connections, run the following query:
List [[Glossary Terms|glossary terms]] To retrieve all available [[Glossary Terms|glossary terms]], run the following query. The query returns the identifier and name of each glossary term.
...
List monitoring projects • TO run a [[Monitoring Project|monitoring project]], you need the [[Monitoring Project|monitoring project]] identifier (projectld). • All project identifiers can be retrieved through the following query. GraphQL query for listing monitoring projects query getProjects { monitoringProjects(versionSeIector. { draftVersion: true { edges { node { gid


Retrieve occurrences • TO get an overview Of the entities to which a specific glossary term has been assigned, run the following query. The term identifier (gid) can be obtained using the previous query for listing term types. When the withCount argument is set to true, the total count also includes the occurrences on child entities. For example, this means that terms assigned to attributes are also counted for the [[Catalog Items|catalog items]] containing those attributes. To calculate occurrences of only directly assigned terms (that is, to exclude catalog item counts), set the value to false. The query returns a list of available entities and the number Of times the term has been assigned to each entity type.
...
This query aggregationResult returns DQ aggregated result for a term. aggregationResult lives under the term entity. It takes the last results from all attributes where the term is assigned and aggregates them. Parameters: • at - specifies what moment in history we are interested. All runs after this time are excluded from the aggregation.
...
Profile [[Catalog Items|catalog items]] TO start [[Profiling|profiling]] [[Catalog Items|catalog items]], you first need to retrieve all available published [[Profiling|profiling]] configurations, using the following query. There are two default [[Profiling|profiling]] configurations: FULL and SAMPLE. If there are no user-defined [[Profiling|profiling]] configurations, the listProfilingaConfigurations query returns only these two options. A [[Profiling|profiling]] configuration needs to be published before it can applied during [[Profiling|profiling]]. To specify which [[Profiling|profiling]] configuration you want to use, you need to provide the unique identifier of that configuration (gid). The listProfiIingConfigurations query also returns details about the configuration itself, such as name, type, description, sampling limits, and information about [[Anomaly Detection|anomaly detection]].

Profile [[Catalog Items|catalog items]] • The query triggers [[Profiling|profiling]] on a catalog item. • These arguments are required: gid on the catalogltem field. Refers to the unique identifier of the catalog item that should be profiled. configld on the profile field. Points to the identifier (gid) of the selected [[Profiling|profiling]] configuration, which was obtained using the listProfilingConfigurations query.

Profile [[Catalog Items|catalog items]] TO check the job status after the [[Profiling|profiling]] has started, run the following query. The gid argument on the job field matches the gid field on the profile item that is found in the results returned by the [[Profiling|profiling]] query.