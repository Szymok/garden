...
- **Wysoka skuteczność:** PAL przewyższa nawet większe modele z klasycznym [[Chain-of-Thought Prompting|CoT]] w zadaniach BIG-Bench Hard i GSM8K — jest obecnie stanem sztuki w reasoning code-based.
...
2. **Interpretacja przez [[Base LLM|LLM]]** — model rozbija problem na etapy, generując kolejne linie kodu i opatrując je logicznymi komentarzami (każdy krok ma opis po polsku lub angielsku, poprzedzony "#").
...
### Kluczowa różnica z Chain-of-Thought ([[Chain-of-Thought Prompting|CoT]])

|Technika|Sposób rozumowania|Sposób uzyskania odpowiedzi|
|---|---|---|
|[[Chain-of-Thought Prompting\|CoT]]|krok po kroku, tekstowo|model sam prowadzi rozumowanie i generuje odpowiedź|
|**PAL**|generacja kodu z komentarzami|model generuje kod, który następnie wykonuje interpreter|

- **PAL** minimalizuje ryzyko błędów logicznych i arytmetycznych, które często pojawiają się przy czysto tekstowym rozumowaniu ([[Chain-of-Thought Prompting|CoT]]).
...
- Modelowi [[Base LLM|LLM]] zamienić pytanie w kod z logicznymi komentarzami.
...
**Efekt:** Złożone zadania matematyczne, manipulacje datami, operacje na strukturach danych rozwiązywane są precyzyjnie — model nie popełnia typowych błędów [[Base LLM|LLM]] dzięki delegowaniu obliczeń do kodu.
...
- Superiority nad [[Chain-of-Thought Prompting|CoT]] (błąd arytmetyczny/logiczy vs. deterministyczność kodu).
...
- [[Base LLM|LLM]] nie musi “znać” całego rozwiązania — przekłada problem na kod.