...
- Wspierana przez formaty tabel warstwowych z **metadanymi i logiem zmian** (np. DeltaLog, [[Apache Iceberg|Iceberg]] manifest).
...
W formatach [[Data Lakehouse|lakehouse]] (Delta, [[Apache Iceberg|Iceberg]], [[Apache Hudi|Hudi]]) jest to rozwiązane przez **warstwę metadanych**, która umożliwia zastosowanie zmian bez konieczności modyfikacji już istniejących plików z danymi.
...
- Delta / [[Apache Iceberg|Iceberg]] / [[Apache Hudi|Hudi]] mają różny poziom wsparcia
- Istotne w strumieniach, ingestion pipeline, logach systemowych
- Time travel + schema versioning → warunek dobrego lineage
- Dobrze łączy się z [[Transakcje ACID|ACID]] → dane nadal spójne pomimo ewolucji
