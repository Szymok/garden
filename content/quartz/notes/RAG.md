...
**RAG ([[Retrieval Augmented Generation (RAG)|Retrieval-Augmented Generation]])** to strategia łączenia dużych modeli językowych ([[Base LLM|LLM]]) z własnymi, najnowszymi, często prywatnymi zbiorami danych, bez potrzeby dalszego trenowania modelu. RAG rozszerza możliwości [[Base LLM|LLM]], które zostały zatrzymane na wybranym punkcie w czasie (np. [[GPT-4]] do 2021 roku), pozwalając na dynamiczne „zasilanie” modeli aktualną wiedzą oraz eliminację efektu tzw. halucynacji.

# 🔑 Kluczowe punkty

- RAG umożliwia **aktualizowanie wiedzy modelu [[Base LLM|LLM]] na bieżąco** poprzez dynamiczne podłączanie nowych danych – bez kosztownego retreningu.
...
4. **Generowanie odpowiedzi**:  
    Odpowiadające fragmenty dołączane są jako „kontekst” (in-context) do promptu dla [[Base LLM|LLM]]. Model generuje odpowiedź, opierając się nie tylko na własnej wiedzy, ale i na znalezionych, aktualnych danych.
...
|Generacja|[[Base LLM\|LLM]] (np. OpenAI [[GPT-4]])|Odpowiedz, korzystając z retrieval context|
...
- OpenAI Cookbook – [[Retrieval Augmented Generation (RAG)|Retrieval-Augmented Generation]] with GPT-3: [https://github.com/openai/openai-cookbook/blob/main/examples/Retrieval_augmentation_for_QA.ipynb](https://github.com/openai/openai-cookbook/blob/main/examples/Retrieval_augmentation_for_QA.ipynb)
...
- Przykład: pytanie → [[embedding]] → query do wektorowej → doc context → prompt do [[Base LLM|LLM]] → odpowiedź oparta na własnych dokumentach.
