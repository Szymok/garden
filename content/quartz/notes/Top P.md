...
**Top P**, znany również jako **nucleus sampling**, to technika generowania tekstu wykorzystywana w dużych modelach językowych ([[Base LLM|LLM]]), polegająca na ograniczaniu zbioru możliwych tokenów do takiego, który zawiera tylko te tokeny, których łączne prawdopodobieństwo przewyższa zadany próg *p* (np. 0.9). Spośród tego podzbioru token wybierany jest losowo — zamiast zawsze wybierać najbardziej prawdopodobny token.
...
[1] Hugging Face — [[Top K|Top-K sampling]] and Top-P (nucleus) sampling: https://huggingface.co/blog/how-to-generate  
