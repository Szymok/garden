...
**[[Keycloak]] [[Roles]]** to zestaw ról bezpieczeństwa w systemie [[Keycloak]], przypisywanych użytkownikom lub grupom w celu definiowania poziomów uprawnień w aplikacjach i modułach. W kontekście platformy Ataccama ONE, [[Roles|role]] zdefiniowane w [[Keycloak]] są wykorzystywane do inicjalnej autoryzacji użytkowników i mogą integrować się z rolami governance oraz dostępem do danych i modułów aplikacyjnych.

# 🔑 Kluczowe punkty

- [[Roles|Role]] [[Keycloak]] są podstawą pierwszego poziomu kontroli dostępu w Ataccama ONE (IAM).
- [[Roles|Role]] mogą być **kompozytowe** — przypisanie jednej roli powoduje nadanie wielu ról wewnętrznych pakietowo.
- [[Roles|Role]] są powiązane z konkretnymi komponentami/systemami (np. MMM, DPP, RDM).
...
## Główne [[Roles|role]] systemowe [[Keycloak]]
...
|**MMM_user**|Uprawnia do pierwszego logowania. Nadaje rolę „Data Consumer” w grupie domyślnej. Dostęp utrzymywany nawet po usunięciu tej roli, jeśli użytkownik ma inne [[Roles\|role]] governance.|

> Rola MMM_user jest często stosowana jako domyślna dla użytkowników logujących się przez zewnętrzny provider tożsamości (np. LDAP, Azure AD) – dopóki nie zostaną przypisane [[Roles|role]] właściwe dla domen czy grup.
...
| [[Master Data Management (MDM)\|Master Data Management]] | [[Master Data Management (MDM)\|MDM]] | `Master Data Management (MDM)\|MDM_admin`, `MDM_edit` |
...
[[Roles|Role]] z Keycloak mogą współistnieć z rolami nadawanymi w systemie ONE (governance [[roles]]):
...
2. Administrator przypisuje mu dodatkowe [[Roles|role]] governance w zależności od kontekstu (grupowego, domenowego lub technicznego).
...
- Trzy kluczowe [[Roles|role]] w MMM: user, admin, application admin
- Rola `MMM_user` = bilet wejścia, potem można odebrać, użytkownik zostaje dzięki governance
- [[Roles|Role]] "systemowe" integrują się z modulem zarządzania uprawnieniami w ONE — dostęp na poziomie UI, API, katalogu metadanych
- "Composite [[roles]]" = zewnętrzne [[Roles|role]] mogą pakować wewnętrzne zestawy ról (np. `keycloak_infra_admin` → `MMM_admin + CS_admin`)
